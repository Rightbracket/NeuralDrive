...
In addition to the detection script, the system includes `/etc/modules-load.d/nvidia-uvm.conf`. This file contains `nvidia-uvm` to ensure the module is automatically loaded at boot.

## Ollama Service Integration

As a safety net, the Ollama systemd unit also includes `ExecStartPre` commands for both `modprobe nvidia-uvm` and `nvidia-modprobe -u`. This ensures the necessary drivers and device nodes are present even if the primary detection service is delayed.
